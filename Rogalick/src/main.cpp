...
int main()
{
 ...
 // player.width = 20;
      ...
      if ((time_count - time_begin_3).asSeconds() >= dt) {
        ...
        for (int i = 0; i < particles.size(); i++) {
            ...
            for (int j = 0; j < walls.size(); j++) {
                ...
                if (Collisions::collisionOfBallWithRect(walls[j].rectangle, &(particles[i]))) {
  ...
  for (int i = 0; i < walls_without_collisions.size(); i++) {
     ...
     // if ((player.position.x + player.width + 1 > walls_without_collisions[i].position.x)) {
      //    player.velocity.y = 0;
      //    player.position.x = walls_without_collisions[i].position.x;
      //}
  }
