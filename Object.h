...
public:
    ...
    void TeleportHero(PlayerAnimation* a) {
        if (a->body.getPosition().x > PositionConnect.x + a->body.getSize().x && a->body.getPosition().x < PositionConnect.x + 30 && a->body.getPosition().y > PositionConnect.y + a.body.getSize().y && a->body.getPosition().y < PositionConnect.y) {
            if (Direction == 1) {
                a->position.x = position.x + 30;
            }
            else {
